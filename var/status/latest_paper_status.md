# FTT Paper Status

## Что Происходит

ALPACA PAPER | ЕСТЬ ОТКРЫТЫЕ ЗАЯВКИ У БРОКЕРА
Снимок: 2026-09-17 11:15:32 Нью-Йорк
У брокера: позиций 1, открытых заявок 1.
Капитал $27,575.08; деньги на счёте $26,583.66.
Лимит покупок брокера $54,158.74: может включать заёмные средства, это не бюджет новой сделки.
Заявка: продажа MSFT; количество 2, исполнено 0. Наличие заявки не означает полного исполнения.
Статус ACTIVE: учёт стратегии активен, но это не разрешение покупать.
Допуск нового выпуска: только экспериментальный paper. validation_selected=false: строгий исторический отбор не пройден; это не live-допуск.

Бот: новые покупки сейчас не разрешены или разрешение не подтверждено.
Почему: Есть заявка в исполнении или сверке; новый дополнительный риск заблокирован.
Что дальше: Дождаться ответа брокера и сверки фактического количества.
Почему: План не находится в разрешённом окне исполнения.
Что дальше: Дождаться подходящего свежего сигнала; прошедшую цену не догонять.

Расчёт модели: закрытие 2026-09-16; план на 2026-09-17.
На наблюдении: MSFT. Это лидеры рейтинга, НЕ заявка и НЕ факт покупки.
Частей стратегии: 8 | модель не планирует новую покупку.
Частей стратегии: 4 | модель предлагает пересмотр позиции; допуск проверяется отдельно.
Ручных действий по этому сообщению не требуется. Сообщение не отправляет заявки.

## Технические Подробности

- Generated: `2026-09-17T15:15:32+00:00`
- Market open: `yes`
- Orders enabled: `yes`
- Paper account guard: `verified`
- New production entries: `blocked`
- Entry block reason: `author_style_unqualified_tactical_rotation_shadow_only_2026-07-16`
- Equity: `$27,575.08`
- Cash: `$26,583.66`
- Buying power: `$54,158.74`
- Hybrid-v4 runtime: `active`
- Hybrid-v4 health: `healthy`
- Telegram outbox: `0 pending, 0 failed pending, 109 delivered`
- Hybrid reconciliation: `reconciled`
- Hybrid entry/add now: `blocked`
- Hybrid entry/add reasons: `Отправка и исполнение проверяются отдельно от целей модели.`
- Telegram opening report key: `portfolio-open:d1d8ddc9925b:2026-09-17:v3`
- Telegram close report key: `portfolio-close:hybrid-v4-bull5-2026-09-15:2026-09-16`
- Live review not before: `2026-10-16T13:41:18+00:00`

## Positions
- `MSFT` qty `2`, avg `$492.00`, price `$495.71`, value `$991.42`, P/L `$7.42` (`+0.75%`), today `+1.10%`

## Open Orders
- `MSFT` sell stop qty `2`, limit `-`, status `new`

## Recent Actions
- `2026-09-17T15:15:09` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:14:07` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:13:04` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:12:01` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:10:59` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:09:56` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:08:54` `-` `monitor_heartbeat`: details_redacted_use_local_logs
- `2026-09-17T15:07:52` `-` `monitor_heartbeat`: details_redacted_use_local_logs
