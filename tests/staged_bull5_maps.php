...
$check(is_file(dirname(__DIR__) . '/stage_sources.json') && !is_file(dirname(__DIR__) . '/.env'), 'Only isolated credential-free stage.');
$check(isset($files['src/Research/CandidateBullRiskStudy.php']) && !isset($files['src/Research/CandidateResearchReplay.php']), 'Stage hashes signal rules but excludes replay runner.');
$selected = B::maps($cases['bull_v110_ma50_boost105'], $all, $breadth, $vvix);
$wired = \FulltimeTrading\Paper\CandidateDefinition::indicatorMaps($all, $breadth, $vvix);
$check($wired['scale'] === $selected['scale'] && $wired['confirm']['volatility_calm'] === $selected['confirmation'] && $wired['bullish'] === $selected['bullish'], 'Runtime wires complete declared maps, including positive bull branch.');
$check(\FulltimeTrading\Paper\CandidateDefinition::INDICATOR_RECIPE === 'bull_v110_ma50_boost105', 'Exact staged recipe.');
echo "Staged bull-risk maps: $n assertions PASS\n";
