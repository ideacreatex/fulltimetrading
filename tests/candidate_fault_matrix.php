...
for ($seed = 1; $seed <= 100; ++$seed) {
    ...
    $broker = new class($seed) implements CandidateBrokerGateway {
    ...
    try {
        ...
        $ledger->provision(['run_id' => 'fault', 'profile' => 'candidate', 'strategy_hash' => str_repeat('a', 64), 'runtime_hash' => str_repeat('b', 64),
            'data_contract' => ['execution_contract' => Order::CONTRACT, 'paper_only' => true]], array_fill_keys($names, 1 / 12));
        $ledger->activate('fault', 30000., ['positions' => [], 'open_orders' => [], 'adoption' => 'flat_account_only', 'stable_for_seconds' => 120]);
        $plans = []; $count = 2 + $seed % 7;
        foreach ($names as $index => $name) { $plans[$name] = ['target_quantities' => $index < $count ? [['AAA', 'BBB', 'CCC'][$index % 3] => mt_rand(2, 15)] : null,
            'target' => ['risk_exit_pending' => false], 'circuit_feedback' => ['force_cash' => false, 'scale' => 1.]]; }
        $close = ['date' => '2026-09-14', 'scheduled_session' => '2026-09-15', 'plans' => $plans, 'provenance' => ['fixture_seed' => $seed]];
...
echo "candidate_fault_matrix: 100 deterministic scenarios, {$n} assertions PASS\n";
