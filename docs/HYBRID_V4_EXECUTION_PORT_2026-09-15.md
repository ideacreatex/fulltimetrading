...
Deployed to the existing Alpaca paper LaunchAgent at 2026-09-15T01:51:22Z.
This is an experimental paper release, not a historical-validation PASS or live approval.
The predecessor's history and stored identities are preserved; its run is now paused.
...
The existing `bin/install-hybrid-launchd` completed successfully in candidate-v1
mode. No second trading agent was created. The replacement daemon has PID 79845;
the exits-only legacy daemon remains PID 1477. Both have matching held locks,
fresh heartbeats and verified paper-account guards.

- Run: `hybrid-v4-candidate-2026-09-15`, profile `maximum-stop12-costband2-whole-v1`, twelve sleeves.
- Actual activation: `2026-09-15T01:51:22+00:00`; entry commissioning: `01:51:26+00:00`.
- Runtime hash: `709c3979f85a08c8e84eb9ee2f94598d2a0698ddb26e3b9e5d34c00c0ae341dd`.
- The real broker accepted four automatic next-open MSFT orders: 9 + 3 + 3 + 3 shares.
- At `01:54:36Z`: all four are `accepted`, OPG, zero filled shares and zero positions.
- Equity/cash: $27,567.66; buying power after order reservations: $46,056.08; actual P/L/drawdown: zero.
- Reconciliation passes, each intent has one submission attempt, no duplicate orders or fill-audit rows.
- Telegram: 97 delivered, zero pending/failed; current close and activation messages delivered.
- Status export passes and publishes the actual new run. Every candidate asset is excluded from legacy ownership.
- Earliest calendar review: `2026-10-16T01:51:22+00:00`, still requiring 20 market dates and all forward criteria.

The local release reports retain the pre-handoff backup, isolated fresh-broker
preflight and post-handoff status/month snapshots. Tests sent no broker orders;
the four accepted orders were generated by the installed paper daemon only after
commissioning. No live activation, manual order, position liquidation, API host
change or credential change was performed. Accepted orders are not executions.
