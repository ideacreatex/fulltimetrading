...
$root = dirname(__DIR__); $capital = (float) ($argv[1] ?? '27567.66');
if (!is_finite($capital) || $capital < 1000 || $capital > 1000000) { throw new InvalidArgumentException('Invalid capital sensitivity.'); }
$out = $root . '/var/reports/candidate_capital_' . str_replace('.', '_', number_format($capital, 2, '.', '')) . '_20260915';
...
$protocol = ['end' => '2026-09-04', 'starts' => ['continuous' => '2021-01-04', 'fresh2023' => '2023-01-03'],
    'costs_bps' => [30, 60], 'variants' => ['maximum', 'candidate'], 'sizing' => ['whole_previous_close'],
    'initial_equity' => $capital, 'cost_band_stress' => 'Strength 1 at 60bps preserves strength 2 at 30bps switching hurdle.',
...
foreach (['src/Research/PaperExecutionRotationBacktester.php', 'src/Research/PaperExecutionRotationEnsembleBacktester.php',
    'src/Trading/WholeShareSizing.php', 'tools/replay_candidate_capital.php'] as $file) { $protocol['code_sha256'][$file] = hash_file('sha256', $root . '/' . $file); }
        ...
        foreach ($protocol['costs_bps'] as $cost) {
            foreach ($protocol['variants'] as $variant) {
                foreach ($protocol['sizing'] as $sizeMode) {
                    ...
                    $result = $engine->runControlled($bars, $start, $protocol['end'], $controller, $capital);
