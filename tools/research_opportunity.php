...
if ($mode === 'prepare') {
    ...
    $cases = O::definitions(); foreach ($cases as $d) { O::validate($d['policy']); }
    $features = O::features($all, $vvix, $base['original_universe']);
    $p = ['frozen_at' => gmdate(DATE_ATOM), 'cases' => $cases, 'cost_bps' => 30, 'initial_equity' => 30000,
        'starts' => ['2021-01-04', '2023-01-03'], 'end' => '2026-09-04', 'phases' => [0, 1, 2],
        'selection' => 'One per family by worst-phase train 2021-2023 Calmar; also audit two unchanged controls. No hindsight extra leaders.',
...
if ($mode === 'select') {
    ...
    $ids = array_values(array_unique(array_merge(['maximum', 'maximum_stop12'], array_values($selected)))); $jobs = [];
    foreach ($ids as $id) {
        foreach (['cost60_2021', 'cost60_2023', 'expanded'] as $scenario) { foreach ([0, 1, 2] as $phase) { $jobs[] = compact('id', 'scenario', 'phase'); } }
...
$jobs = [];
if ($mode === 'worker') { foreach (array_keys($p['cases']) as $id) { foreach (['continuous', 'fresh2023'] as $scenario) { foreach ([0, 1, 2] as $phase) { $jobs[] = compact('id', 'scenario', 'phase'); } } } }
...
foreach ($jobs as $i => $job) {
    ...
    $changes = S::merge($changes, ['rebalance_phase' => $phase]);
    ...
    if ($p['cases'][$id]['family'] === 'control' && in_array($scenario, ['continuous', 'fresh2023'], true) && $phase === 0) {
        $oldMatch = $curve == $read($root . '/var/reports/selected_maximum_20260909/' . ($scenario === 'continuous' ? 'full2021' : 'primary') . '_' . $id . '_curve.json');
        if (!$oldMatch) { throw new RuntimeException('Original curve does not reproduce.'); }
    }
