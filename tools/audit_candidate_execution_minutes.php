...
$root = dirname(__DIR__); $dir = $root . '/var/reports/candidate_execution_20260915'; $out = $dir . '/minutes';
...
foreach (['continuous', 'fresh2023'] as $scenario) {
    $file = $dir . '/' . 'close_parity_' . $scenario . '.json'; $source = $read($file); $inputs[$scenario] = hash_file('sha256', $file);
    if (!$source['exact_frozen_metrics']) { throw new RuntimeException('Candidate replay not verified.'); }
...
$protocol = ['input_sha256' => $inputs, 'script_sha256' => hash_file('sha256', __FILE__), 'events' => count($events),
    'symbol_days' => array_sum(array_map('count', $byDate)), 'feed' => 'Alpaca SIP adjustment=split',
...
$provider = new AlpacaBarsProvider($http, 'https://data.alpaca.markets', 'sip', 'split', 10000);
