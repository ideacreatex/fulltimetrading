...
$options = [
    ...
    'positions' => __DIR__ . '/../var/reports/author_grid/best_positions.json',
    'output' => __DIR__ . '/../var/reports/author_grid/telegram_position_comparison.json',
...
$positionsPayload = json_decode((string) file_get_contents((string) $options['positions']), true, 512, JSON_THROW_ON_ERROR);
$events = $telegram['events'] ?? [];
$positions = $positionsPayload['positions'] ?? [];
if (!is_array($events) || !is_array($positions)) {
...
foreach ($positions as $position) {
    if (!is_array($position) || !isset($position['symbol'], $position['date'])) {
        continue;
    }
    ...
    if ($family !== null) {
        $positionsByFamily[$family][] = $position;
    }
}
...
foreach ($events as $event) {
    ...
    foreach ($tickers as $ticker) {
        ...
        $exactMatches = annotateMatches(
            matchingPositions($positionsBySymbol[$ticker] ?? [], (string) $event['date'], $windowDays),
            'exact',
            $family,
        );
        $familyMatches = [];
        if ($familyMatchEnabled && $family !== null) {
            $familyMatches = annotateMatches(
                matchingPositions($positionsByFamily[$family] ?? [], (string) $event['date'], $windowDays),
                'family',
                $family,
            );
        }
        ...
        $familyOnlyMatched = $matches !== [] && !$exactMatched;
        ...
        if ($familyOnlyMatched) {
            $familyOnlyMatchedTickerRows++;
        ...
        $rows[] = [
            ...
            'family_matched' => $familyOnlyMatched,
...
$result = [
    ...
    'position_variant' => $positionsPayload['variant'] ?? null,
    'authors' => $authors,
    ...
    'summary' => [
        ...
        'family_only_matched_ticker_rows' => $familyOnlyMatchedTickerRows,
        'family_only_ticker_match_rate' => $tickerRows > 0 ? $familyOnlyMatchedTickerRows / $tickerRows : 0.0,
        'position_rows' => count($positions),
...
function matchingPositions(array $positions, string $eventDate, int $windowDays): array
{
    $event = new DateTimeImmutable($eventDate);
    $byPosition = [];
    foreach ($positions as $position) {
        $positionDate = new DateTimeImmutable((string) $position['date']);
        $diff = (int) $event->diff($positionDate)->format('%r%a');
        if (abs($diff) > $windowDays) {
            continue;
        }
        $position['days_from_message'] = $diff;
        $key = (string) ($position['key'] ?? implode(':', [
            (string) ($position['symbol'] ?? ''),
            (string) ($position['entry_date'] ?? ''),
            (string) ($position['strategy'] ?? ''),
        ]));
        if (!isset($byPosition[$key]) || abs($diff) < abs((int) $byPosition[$key]['days_from_message'])) {
            $byPosition[$key] = $position;
        }
    }

    $matches = array_values($byPosition);
    usort($matches, static fn (array $a, array $b): int => abs((int) $a['days_from_message']) <=> abs((int) $b['days_from_message']));
