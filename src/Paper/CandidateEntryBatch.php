...
final readonly class CandidateEntryBatch
{
    ...
    public function next(string $run, array $close, array $window, array $guards): ?array
    {
        ...
        $inWindow = (($window['opg_submit_allowed'] ?? false) || ($window['rotation_reentry_allowed'] ?? false))
        ...
        if ($observedFill || $state['aborted'] !== null || $state['phase'] === 'protecting') {
            ...
            foreach ($buys as $i) { return ['action' => 'cancel', 'decision_id' => $i['decision_id'], 'reason' => 'partial_fill_protection']; }
