 ...
 * child remains a shadow-only PaperExecutionRotationBacktester.
 */
final class PaperExecutionRotationEnsembleBacktester
{
    /**
     * @var array<string,array{
     *     allocation:float,
     *     backtester:PaperExecutionRotationBacktester
     * }>
     */
    ...
    public function __construct(
        ...
        foreach ($sleeves as $key => $definition) {
            ...
            $normalized[$name] = [
                ...
                'backtester' => new PaperExecutionRotationBacktester($config, $indicators),
            ];
    ...
    /** Shared portfolio feedback is calculated only after all sleeves finish the same session. */
    public function runControlled(array $bars, string $start, string $end, callable $controller, float $initialEquity = 30000.0): array
    {
        if (!is_finite($initialEquity) || $initialEquity <= 0) { throw new \InvalidArgumentException('Invalid initial capital.'); }
        $iterators = $initials = [];
        $allocated = 0.0;
        $last = array_key_last($this->sleeves);
        foreach ($this->sleeves as $name => $sleeve) {
            $initials[$name] = $name === $last ? $initialEquity - $allocated : $initialEquity * $sleeve['allocation'];
            $allocated += $initials[$name];
            $iterators[$name] = $sleeve['backtester']->steps($bars, $start, $end, $initials[$name]);
        }
        while (reset($iterators)->valid()) {
            $rows = [];
            $date = null;
            foreach ($iterators as $name => $iterator) {
                if (!$iterator->valid()) { throw new \RuntimeException('Controlled sleeve ended early.'); }
                $rows[$name] = $iterator->current();
                $date ??= $rows[$name]['date'];
                if ($date !== $rows[$name]['date']) { throw new \RuntimeException('Controlled sleeve calendar mismatch.'); }
            }
            $feedback = $controller($date, $rows);
            foreach ($iterators as $iterator) { $iterator->send($feedback); }
        }
        $sleeveResults = $sleeveCurves = $nextTargets = [];
        foreach ($iterators as $name => $iterator) {
            if ($iterator->valid()) { throw new \RuntimeException('Controlled sleeve ended late.'); }
            $child = $iterator->getReturn();
            $allocation = $this->sleeves[$name]['allocation'];
            $sleeveCurves[$name] = $child['curve'];
            $nextTargets[$name] = array_merge($child['next_target'], ['allocation' => $allocation,
                'initial_equity' => $initials[$name], 'capital_scope' => 'independent_static_sleeve', 'shadow_only' => true]);
            $sleeveResults[$name] = array_merge($child, ['allocation' => $allocation, 'initial_equity' => $initials[$name]]);
        }
        $asOf = array_map(static fn ($r): string => $r['features_as_of'], $sleeveResults);
        return ['curve' => $this->aggregateCurves($sleeveCurves, $sleeveResults), 'sleeve_curves' => $sleeveCurves,
            'sleeves' => $sleeveResults, 'circuit_activations' => array_sum(array_column($sleeveResults, 'circuit_activations')),
            'position_exit_activations' => array_sum(array_column($sleeveResults, 'position_exit_activations')),
            'next_targets' => $nextTargets, 'features_as_of' => count(array_unique($asOf)) === 1 ? reset($asOf) : null,
            'features_as_of_by_sleeve' => $asOf, 'initial_equity' => $initialEquity,
            'capital_model' => 'independent_static_sleeves', 'shadow_only' => true, 'order_submission_enabled' => false];
    }
